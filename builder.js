...
function recordedEventToBlock(ev) {
  ...
  if (ev.type === 'fill') return { id, type:'fill', target, value: ev.sensitive ? '' : String(ev.value ?? ''), timeoutMs:5000, fillMode:'framework', blurAfter:true, shadowSearch:true, recorded:true };
...
function blockInline(b) {
  ...
  if (b.type === 'selectOption') return `${inlinePick(b, 'Dropdown')} ${inlineInput('optionText', b.optionText || '', 'opció szövege', 'wide')} ${inlineSelect('matchMode', b.matchMode || 'contains', [['contains','tartalmazza'],['equals','pontosan']])}`;
