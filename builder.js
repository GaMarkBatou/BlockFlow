...
const BLOCK_HELP = {
  ...
  textSearch: { purpose:'Egyszerű szöveget keres az oldalon regex nélkül, és visszaadja azt is, hogy hol találta meg.', params:['Keresett szöveg: a keresendő szó vagy mondatrész.','Hol keressen: látható szövegben, teljes DOM-ban vagy mezőértékekkel/attribútumokkal együtt.','Eredmények: igaz/hamis, találatszám, környező szöveg, selector, XPath, találati sor, kattintható szülő és közeli gomb változókba kerül.'] },
...
function renderInspector() {
  ...
  if (b.type === 'scroll') html += selectField('mode','Görgetés módja', b.mode || 'element', [['element','Elemhez görget'],['page','Oldal/konténer görgetése']]) + selectField('scrollTarget','Görgetési cél', b.scrollTarget || 'auto', [['auto','Automatikus'],['page','Teljes oldal'],['nearest','Cél elem legközelebbi görgethető konténere'],['container','Kézzel kiválasztott görgethető konténer']]) + (b.scrollTarget === 'container' ? '<div class="field"><label>Görgethető konténer</label><button class="btn pick-btn" data-pick="scrollContainer">Elem kiválasztása</button></div>' : '') + selectField('align','Elem igazítása', b.align || 'center', [['center','Középre'],['top','Felülre'],['bottom','Alulra']]) + selectField('direction','Irány', b.direction || 'down', [['down','Le'],['up','Fel'],['top','Tetejére'],['bottom','Aljára']]) + numberField('amount','Mennyiség px', b.amount || 500) + '<div class="status">Automatikus módban a blokk megkeresi a cél elem legközelebbi belső görgethető konténerét, és azt görgeti, nem feltétlenül az egész oldalt.</div>';
...
function updateField(field, value) {
  ...
  renderVariables();
  renderImportWarning();
  if (['waitMode','conditionMode','maskMode','scope','domain','path','url','urlContains','logic','operator','changeMode','readMode','searchScope','firstRun','pageSize','orientation','source','action','style','align','soundSource','rowMode','columnMode','scrollTarget','mode'].includes(field)) renderInspector();
}
