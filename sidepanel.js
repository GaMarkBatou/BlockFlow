...
async function run(dryRun=false, forceRun=false){ await refreshTarget(); const wf=workflows.find(w=>w.id===$('#workflowSelect').value); if(!wf) return; const validation = BF.validateWorkflow(wf); if(!dryRun && !validation.ok){ $('#log').textContent='Ellenőrzési hiba:\n'+validation.issues.map(i=>`${i.level}: ${i.text}`).join('\n'); return; } if(!dryRun && validation.issues.length && !confirm('Az ellenőrzés figyelmeztetéseket talált. Folytatod?')) return; if(!dryRun && wf && wf.verified===false && !confirm('Ez importált vagy nem ellenőrzött automatizmus. Javasolt előbb Dry-run módban tesztelni. Mégis futtatod?')) return; $('#log').textContent=forceRun?'Kényszerített futtatás...':(dryRun?'Dry-run...':'Futtatás...'); const res=await BF.sendToTarget({type:'BF_RUN_WORKFLOW', workflow:wf, options:{dryRun, forceRun}}, targetTabId); $('#log').textContent=res.response?.ok ? `${res.response.result.skipped?'Nem indult el':'Kész'}.${dryRun?' [dry-run]':''}${forceRun?' [force]':''}\n${JSON.stringify(res.response.result.vars,null,2)}\n\n${(res.response.result.log||[]).join('\n')}` : `Hiba: ${res.response?.error || res.error}\n\n${(res.response?.log||[]).join('\n')}`; }
